...
struct Process
{
    ...
    int *waiting;    // Pointer to an array to store Process IDs of processes this process is waiting for
...
int main()
{
    ...
    for (int i = 0; i < process; i++)
    {
        ...
        printf("How many processes is process-%d waiting for (Enter -2 if none)? : ", i + 1);
        scanf("%d", &pList[i].num_waiting);

        // Allocate memory for waiting array
        pList[i].waiting = (int *)malloc(MAX_WAITING * sizeof(int));
    ...
    for (int i = 0; i < process; i++)
    {
        free(pList[i].waiting); // Free waiting array for each process
    }
    free(pList); // Free the array itself
